 ...
 */
void index_html(httpd *server, httpReq *request)
{
    httpdPrintf(server, request,
                (char*)"Welcome to the httpd server running in process number %d<P>\n",
                getpid());
    httpdPrintf(server, request,
                (char*)"Click <A HREF=/test1.html>here</A> to view a test page<P>\n");
    httpdPrintf(server, request,
                (char*)"Click <A HREF=/login.html>here</A> to authenticate<P>\n");
    httpdPrintf(server, request,
                (char*)"Or <A HREF=/wildcard/foo>here</A> for a test wildcard page<P>\n");
    httpdPrintf(server, request,
                (char*)"<P><FORM ACTION=test2.html METHOD=POST>\n");
    httpdPrintf(server, request,
                (char*)"Enter your name <INPUT NAME=name SIZE=10>\n");
    httpdPrintf(server, request,
                (char*)"<INPUT TYPE=SUBMIT VALUE=Click!><P></FORM>\n");
    return;
}

void test2_html(httpd *server, httpReq *request)
{
    httpVar    *variable;
     ...
     */
    variable = httpdGetVariableByName(server, request, (char*)"name");
    if (variable == NULL)
    {
        httpdPrintf(server, request, (char*)"Missing form data!");
        return;
    }
     ...
     */
    httpdOutput(server, request, (char*)"Hello ${name}");
}

void test3_html(httpd *server, httpReq *request)
{
    char    *path;
    
    path = httpdRequestPath(request);
    httpdOutput(server,request, (char*)"Wilcard dynamic request received<P>");
    httpdPrintf(server,request, (char*)"The requested path was %s<P>", path);
}


void login_html(httpd *server, httpReq *request)
{
    if (httpdAuthenticate(server, request, (char*)"LibHTTPD Test") == 0)
        return;
    httpdPrintf(server, request, (char*)"Your username is '%s'<P>\n",
                request->authUser);
    httpdPrintf(server, request, (char*)"Your password is '%s'<P>\n",
                request->authPassword);
    httpdOutput(server, request,
                (char*)"Click <A HREF=login2.html>here</A> to force reauthentication");
    httpdOutput(server, request,
                (char*)".  Use a username = test password = 123");
}


void login2_html(httpd *server, httpReq *request)
{
    if (httpdAuthenticate(server, request, (char*)"LibHTTPD Test") == 0)
    {
        httpdOutput(server, request, (char*)"Authentication failure(1).");
        return;
    }
    if (strcmp(request->authUser, (char*)"test") != 0 ||
        strcmp(request->authPassword, (char*)"123") != 0)
    {
        httpdForceAuthenticate(server, request, (char*)"LibHTTPD Test");
        httpdOutput(server, request, (char*)"Authentication failure (2).");
        return;
    }
    httpdOutput(server, request, (char*)"Your login was accepted.");
}

void myError404Handler(httpd *server, httpReq *request, int error)
{
    httpdOutput(server, request,
                (char*)"Whoa there.  You hit a page that doesn't exist! <P><BR><BR>\n");
    httpdOutput(server, request,
                (char*)"Or in other words : <B>Error 404</B>\n\n");
}
